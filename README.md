...
배포 후 홈에서 **먼저 말 걸어주기 → 켜기**를 누른 뒤, 발송을 직접 불러본다.

```bash
curl -H "Authorization: Bearer $CRON_SECRET" https://<api-도메인>/notifications/dispatch
```

`{"targeted":1,"sent":1,...}` 이면 정상이다. `targeted:0` 이면 조건에 걸린 것이다 —
방금 대화했거나(20시간), 오늘 이미 받았거나, 구독이 없다.
같은 날 두 번 부르면 두 번째는 `targeted:0` 이 나오는 게 맞다.
