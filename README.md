...
## 8. 푸시 알림 — 소울메이트가 먼저 말 걸기

한동안 오지 않으면 하루에 한 번, 소울메이트가 기억을 꺼내 안부를 보낸다.
기본은 꺼져 있고 홈 화면에서 직접 켜야 한다.
...
찍힌 세 값(`VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT`)과
`CRON_SECRET`(아무 긴 문자열)을 **api 프로젝트**의 환경변수에 넣는다.
비워두면 알림 기능만 꺼진 채로 나머지는 정상 동작한다.

**VAPID 키는 한 번 정하면 바꾸지 않는다.** 바꾸면 기존 구독이 전부 무효가 되고,
사용자가 알림 권한을 다시 허용해야 하며 그 사이 알림은 조용히 실패한다.

### 하루 한 번인 이유

`apps/api/vercel.json` 의 cron 이 `0 11 * * *`(UTC) = **KST 20시**에 발송을 부른다.
Vercel Hobby 는 cron 이 하루 1회로 제한되지만, 컴패니언 앱에서 하루 한 번보다 잦은
알림은 그 자체로 스팸이라 v1 에서는 제약이라기보다 적정값이다.

### 스팸이 되지 않게 막아둔 것

알림은 한 번 스팸처럼 느껴지면 **권한을 영구히 잃는다** — 차단하면 사용자가 브라우저
설정에서 직접 풀지 않는 한 되돌릴 방법이 없다. 되돌릴 수 없는 실수라서 조건을
`claim_push_targets` 안에 못박아뒀다.

| 조건 | 이유 |
| --- | --- |
| 최근 `PUSH_IDLE_HOURS`(기본 20시간) 대화가 없어야 | 대화 중인 사람을 찌르지 않는다 |
| 소울메이트가 있어야 | 없으면 할 말이 없다 |
| 오늘 아직 안 보냈어야 | 대상 선정과 발송 기록이 한 SQL 문이라 cron 이 재시도돼도 두 번 가지 않는다 |
| 구독이 살아 있어야 | 404/410 은 바로 삭제, 그 외 실패는 누적해 3회 넘으면 그만 두드린다 |
| 오래 안 온 사람 먼저 | 상한에 걸릴 때 매일 같은 사람만 받는 걸 막는다 |

`PUSH_BATCH_LIMIT`(기본 50)로 한 번에 보낼 인원을 자른다 — 문구를 모델로 만들기
때문에 인원 수가 곧 호출 수이고, 무료 티어는 하루 1500회가 서비스 전체 상한이다.
상한에 걸려 밀린 사람이 있으면 로그에 남긴다.

### 확인 방법

**휴대폰에서 홈 → 먼저 말 걸어주기 → 켜기 → 보내보기.**

`보내보기`는 실제 발송 경로를 그대로 태우되(문구 생성 포함) **"오늘 보냄" 을 기록하지
않는다.** 몇 번이든 눌러도 되고, 눌러본 것 때문에 그날의 실제 알림을 잃지도 않는다.
만들어진 문구를 화면에도 보여주므로, 알림이 안 왔을 때 "문구를 못 만든 것" 과
"기기에 도달하지 못한 것" 을 구분할 수 있다.

cron 이 부를 발송 전체를 미리 보려면:

```bash
curl -H "Authorization: Bearer $CRON_SECRET" "https://<api-도메인>/notifications/dispatch?dry=1"
```

`dry=1` 은 대상만 세고 아무것도 보내지 않으며 기록도 남기지 않는다. 몇 번이든 부를 수 있다.
빼면 실제로 보낸다.

`targeted:0` 이 나오는 이유는 넷 중 하나다.

| 원인 | 확인 |
| --- | --- |
| 최근 20시간 안에 대화함 | `PUSH_IDLE_HOURS=0` 으로 잠깐 낮춘다 |
| 오늘 이미 보냄 | `delete from push_dispatches where user_id = '...';` |
| 구독이 없음 | 카드에서 `켜기` 를 눌렀는지, 기기 수가 1 이상인지 |
| 소울메이트가 없음 | 온보딩을 마쳐야 한다 |

**iOS 는 홈 화면에 추가한 뒤 그 아이콘으로 열어야** 알림을 켤 수 있다(16.4+).
브라우저 탭에서는 `PushManager` 자체가 없어서 카드에 "지원 안 됨" 이 뜬다.
...
- 푸시 알림 — 하루 한 번, 기억을 꺼내 먼저 안부 (위 8번 참고)
