...
### Gemini 키 — 나눠 쓰면 대화 비용이 0이 된다

텍스트와 이미지는 과금 조건이 다르다.

| 용도 | 무료 티어 | 필요한 것 |
| --- | --- | --- |
| 페르소나·대화 (텍스트) | **있음** (1,500회/일) | 키만 있으면 됨 |
| 아바타 (이미지) | **없음** | 결제가 연결된 프로젝트 |

그래서 **Google Cloud 프로젝트를 두 개** 두는 구성이 가능하다.

1. 결제를 연결하지 **않은** 프로젝트에서 키 발급 → `GEMINI_TEXT_API_KEY`
2. 결제를 연결한 프로젝트에서 키 발급 → `GEMINI_IMAGE_API_KEY`

이렇게 하면 대화 비용이 0이 되고, **선불 잔액이 바닥나도 대화는 계속 동작한다** —
잔액이 0이면 그 결제 계정에 연결된 키가 *전부 함께* 막히기 때문에 분리해두는 의미가 크다.

감안할 점: 무료 티어는 **분당 10회가 서비스 전체 상한**이라 동시 사용자 10명 남짓이 한계고,
입력이 모델 학습에 쓰인다(개인정보처리방침에 고지해야 함).

나누지 않을 거면 `GEMINI_API_KEY` 하나만 채우면 된다. 용도별 키가 비어 있으면 이 값을 쓴다.
...
두 용도를 각각 호출해보고 실패 사유를 키/결제/한도/모델ID로 나눠서 알려준다.
텍스트만 실패하면 텍스트 키를, 이미지만 실패하면 이미지 키를 보면 된다.
